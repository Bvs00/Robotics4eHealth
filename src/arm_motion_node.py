...
class ShoulderMotionNode:
    ...
    def lwrist_yaw(self, msg):
        try:
            self.motion_proxy.setAngles(["LWristYaw"], [msg.data[0]], msg.data[1])
        except:
            self.motion_proxy = ALProxy("ALMotion", self.ip, self.port)
            self.motion_proxy.setAngles(["LWristYaw"], [msg.data[0]], msg.data[1])
    
    def rshoulder_roll(self, msg):
        #print("Ricevuto " + str(msg))
        try:
            print("Ricevuto " + str(msg))
            self.motion_proxy.setAngles(["RShoulderRoll"], [msg.data[0]], msg.data[1])
        except:
            print("Errore " + str(msg))
            self.motion_proxy = ALProxy("ALMotion", self.ip, self.port)
            self.motion_proxy.setAngles(["RShoulderRoll"], [msg.data[0]], msg.data[1])
    ...
    def rwrist_yaw(self, msg):
        try:
            self.motion_proxy.setAngles(["RWristYaw"], [msg.data[0]], msg.data[1])
        except:
            self.motion_proxy = ALProxy("ALMotion", self.ip, self.port)
            self.motion_proxy.setAngles(["RWristYaw"], [msg.data[0]], msg.data[1])
    ...
    def start(self):
        ...
        rospy.Subscriber("/arm_rotation/wrist/right/yaw", Float32MultiArray, self.rwrist_yaw)
        rospy.Subscriber("/arm_rotation/wrist/left/yaw", Float32MultiArray, self.lwrist_yaw)
